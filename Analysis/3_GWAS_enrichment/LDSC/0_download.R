...
gwas_indir <- '/mnt/disk5_7T/GWASCatalog_summary2/'
...
url <- paste0(prefix, harmonised$utrl[harmonised$id == trait.ID])
trait_url <- harmonised$utrl[harmonised$id == trait.ID]
file_name <- basename(trait_url)
output_path <- paste0(gwas_indir,file_name)
#system(paste("wget -O", shQuote(output_path), shQuote(url)))
...
# outdir
outdir <- '/mnt/disk5_7T/LDSC/sumstats/'
...
write.table(sumstats, paste0(outdir, trait.ID, '.', harmonised.filter$Trait_mod[harmonised.filter$id == trait.ID], '.sumstats'), sep = '\t', row.names = F, quote = F)
