...
# combine all h2
file <- list.files('/workspace/rsrch1/ychen/Projects/Project03_human_circadian/rQTL/cis_QTL/Result/03_enrich_GWAS/LDSC/02_Heritability/Liver', pattern = 'results')
all <- c()
for(x in file){
  
  tmp <- fread(paste0('/workspace/rsrch1/ychen/Projects/Project03_human_circadian/rQTL/cis_QTL/Result/03_enrich_GWAS/LDSC/02_Heritability/Liver/', x))
...
write.table(all, '/workspace/rsrch1/ychen/Projects/Project03_human_circadian/rQTL/cis_QTL/Result/03_enrich_GWAS/LDSC/heritability_liver_combine.txt', quote = F, sep = '\t', row.names = F)
############################################################################## 
# only keep observed QTLs
all <- fread('/workspace/rsrch1/ychen/Projects/Project03_human_circadian/rQTL/cis_QTL/Result/03_enrich_GWAS/LDSC/heritability_liver_combine.txt')
...
write.table(obs, '/workspace/rsrch1/ychen/Projects/Project03_human_circadian/rQTL/cis_QTL/Result/03_enrich_GWAS/LDSC/observed_QTL_heritability.txt', quote = F, sep = '\t', row.names = F)

obs <-  fread('/workspace/rsrch1/ychen/Projects/Project03_human_circadian/rQTL/cis_QTL/Result/03_enrich_GWAS/LDSC/observed_QTL_heritability.txt')
...
obs$trait.1[obs$trait.1 == 'Linoleic-acid-to-total-fatty-acids-percentage-UKB-data-field-23456'] <- 'Linoleic acid to total fatty acids'
...
obs$trait.1 <- factor(obs$trait.1, levels = rev(c('Apolipoprotein A1','Apolipoprotein B',
  'Cholesterol total','HDL cholesterol', 'LDL weighted GWA', 'Cholesteryl esters', 'VLDL particles', 'large VLDL particles', 'small VLDL particles',
  'Total fatty acid', 'Monounsaturated fatty acid', 'Saturated fatty acid', 'Saturated vs total fatty acids', 'Linoleic acid to total fatty acids', 'Triglycerides')))
write.table(obs, '/workspace/rsrch1/ychen/Projects/Project03_human_circadian/rQTL/cis_QTL/13_Results/06_enrich_GWAS/fig4c_LDSC_heritability.txt', quote = F, sep = '\t', row.names = F)
obs <- fread('/workspace/rsrch1/ychen/Projects/Project03_human_circadian/rQTL/cis_QTL/13_Results/06_enrich_GWAS/fig4c_LDSC_heritability.txt')

str_split_fixed(unique(obs$trait), '\\.', 8)[ ,2]
...
pdf('13_Figures/06_enrich_GWAS/lsdc.pdf', width = 18, height = 2.45, useDingbats = F)
